...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    public void setView(float position, View view, int[] toUse, double[] data, double[] slope, double[] cycle, boolean mForceMeasure) {
        ...
        if (this.mRelativeToController != null) {
            ...
            double d14 = dangle2;
            Double.isNaN(d14);
            float dpos_x = (float) (d12 + (d13 * cos2 * d14));
            double d15 = dry;
            double d16 = dradius;
            ...
            double d20 = dangle2;
            Double.isNaN(d20);
            float dpos_y = (float) (d17 + (d19 * d20));
            v_x = pos_x;
            if (slope.length >= 2) {
                slope[0] = dpos_x;
                slope[1] = dpos_y;
            ...
            float dv_width3 = path_rotate;
            v_height = v_height2;
            if (!Float.isNaN(dv_width3)) {
                float dx = dv_x2 + (dv_width2 / 2.0f);
                float dy = dv_y3 + (dv_height3 / 2.0f);
                double d22 = 0.0f;
                double d23 = dv_width3;
