...
public class ArcCurveFit extends CurveFit {
    ...
    private static class Arc {
        ...
        Arc(int mode, double t1, double t2, double x1, double y1, double x2, double y2) {
            ...
            if (this.linear || Math.abs(dx2) < EPSILON) {
                ...
                if (Math.abs(dy2) >= EPSILON) {
                    ...
                    double d2 = this.mVertical ? -1 : 1;
                    Double.isNaN(d2);
                    this.mEllipseA = d2 * dx2;
                    double d3 = this.mVertical ? 1 : -1;
                    Double.isNaN(d3);
                    this.mEllipseB = dy2 * d3;
        ...
        double lookup(double v) {
            ...
            double length = this.mLut.length - 1;
            Double.isNaN(length);
            double pos = length * v;
            int iv = (int) pos;
            double d = (int) pos;
        ...
        private void buildTable(double x1, double y1, double x2, double y2) {
            ...
            while (i < ourPercent.length) {
                double d = i;
                Double.isNaN(d);
                double length = ourPercent.length - 1;
                Double.isNaN(length);
                double angle = Math.toRadians((d * 90.0d) / length);
            ...
            for (int i3 = 0; i3 < this.mLut.length; i3++) {
                double d2 = i3;
                double length2 = this.mLut.length - 1;
                Double.isNaN(d2);
                Double.isNaN(length2);
                double pos = d2 / length2;
                ...
                if (index >= 0) {
                    ...
                    int p2 = (-index) - 1;
                    double d4 = p1;
                    double d5 = (pos - ourPercent[p1]) / (ourPercent[p2] - ourPercent[p1]);
                    Double.isNaN(d4);
                    double pos2 = ourPercent.length - 1;
                    Double.isNaN(pos2);
                    double ans = (d4 + d5) / pos2;
                    this.mLut[i3] = ans;
                }
            }
        }
