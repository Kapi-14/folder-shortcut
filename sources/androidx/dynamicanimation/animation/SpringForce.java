...
public final class SpringForce implements Force {
    ...
    public float getAcceleration(float lastDisplacement, float lastVelocity) {
        ...
        double d = lastDisplacement2;
        Double.isNaN(d);
        double d2 = (-k) * d;
        double d3 = lastVelocity;
        Double.isNaN(d3);
        return (float) (d2 - (d3 * c));
    }
    ...
    public DynamicAnimation.MassState updateValues(double lastDisplacement, double lastVelocity, long timeElapsed) {
        ...
        if (this.mDampingRatio > 1.0d) {
            ...
            double displacement2 = (Math.pow(2.718281828459045d, this.mGammaMinus * deltaT) * coeffA) + (Math.pow(2.718281828459045d, this.mGammaPlus * deltaT) * coeffB);
            ...
            double displacement3 = ((Math.cos(this.mDampedFreq * deltaT) * lastDisplacement2) + (Math.sin(this.mDampedFreq * deltaT) * sinCoeff)) * Math.pow(2.718281828459045d, (-this.mDampingRatio) * this.mNaturalFreq * deltaT);
            double d2 = (-this.mNaturalFreq) * displacement3 * this.mDampingRatio;
            double d3 = -this.mDampingRatio;
            double lastDisplacement3 = this.mNaturalFreq;
            double pow = Math.pow(2.718281828459045d, d3 * lastDisplacement3 * deltaT);
            double d4 = (-this.mDampedFreq) * lastDisplacement2;
            double cosCoeff = this.mDampedFreq;
            currentVelocity = d2 + (pow * ((d4 * Math.sin(cosCoeff * deltaT)) + (this.mDampedFreq * sinCoeff * Math.cos(this.mDampedFreq * deltaT))));
