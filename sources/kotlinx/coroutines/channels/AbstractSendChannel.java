...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    public final Object mo1663trySendJP2dKIU(E element) {
        ...
        if (result instanceof Closed) {
            return ChannelResult.INSTANCE.m1681closedJP2dKIU(helpCloseAndGetSendException((Closed) result));
        }
    ...
    */
    ...
    private final void helpClose(Closed<?> closed) {
        Object closedList = InlineList.m1699constructorimpl$default(null, 1, null);
        while (true) {
            LockFreeLinkedListNode prevNode = closed.getPrevNode();
            Receive previous = prevNode instanceof Receive ? (Receive) prevNode : null;
            if (previous == null) {
                break;
            } else if (!previous.mo1707remove()) {
                previous.helpRemove();
            } else {
                closedList = InlineList.m1704plusFjFbRPM(closedList, previous);
            }
        }
        if (closedList != null) {
