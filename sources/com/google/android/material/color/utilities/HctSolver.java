...
public class HctSolver {
    ...
    static double chromaticAdaptation(double component) {
        ...
        double signum = MathUtils.signum(component);
        Double.isNaN(signum);
        return ((signum * 400.0d) * af) / (27.13d + af);
    }
        ...
        for (int axis = 0; axis < 3; axis++) {
            if (left[axis] != right[axis]) {
                ...
                for (int i = 0; i < 8 && Math.abs(rPlane2 - lPlane2) > 1; i++) {
                    double d = lPlane2 + rPlane2;
                    Double.isNaN(d);
                    int mPlane = (int) Math.floor(d / 2.0d);
    ...
    static double inverseChromaticAdaptation(double adapted) {
        ...
        double signum = MathUtils.signum(adapted);
        double pow = Math.pow(base, 2.380952380952381d);
        Double.isNaN(signum);
        return signum * pow;
    }
