...
public final class DynamicColor {
    ...
    public double getTone(DynamicScheme scheme) {
        ...
        double fContrast2 = farther.contrastCurve.getContrast(scheme.contrastLevel);
        ...
        if (Contrast.ratioOfTones(bgTone3, fInitialTone) >= fContrast2) {
            fTone = fInitialTone;
        } else {
            fTone = foregroundTone(bgTone3, fContrast2);
        }
        if (decreasingContrast) {
            nTone = foregroundTone(bgTone3, nContrast);
            fTone = foregroundTone(bgTone3, fContrast2);
        ...
        if (50.0d > nTone || nTone >= 60.0d) {
            if (50.0d <= fContrast && fContrast < 60.0d) {
                if (stayTogether) {
                    if (expansionDir > 0.0d) {
                        nTone = 60.0d;
                        fContrast = Math.max(fContrast, 60.0d + (delta * expansionDir));
                    } else {
                        nTone = 49.0d;
                        fContrast = Math.min(fContrast, 49.0d + (delta * expansionDir));
                    }
        ...
        return amNearer ? nTone : fContrast;
    }
