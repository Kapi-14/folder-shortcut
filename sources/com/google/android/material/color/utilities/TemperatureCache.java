...
public final class TemperatureCache {
    ...
    public List<Hct> getAnalogousColors(int count, int divisions) {
        ...
        double d = divisions;
        Double.isNaN(d);
        double tempStep = absoluteTotalTempDelta / d;
        ...
        while (true) {
            ...
            double size = allColors.size();
            Double.isNaN(size);
            double desiredTotalTempDeltaForIndex = size * tempStep;
            boolean indexSatisfied = totalTempDelta >= desiredTotalTempDeltaForIndex;
            int indexAddend = 1;
            while (indexSatisfied && allColors.size() < divisions) {
                allColors.add(hct2);
                double size2 = allColors.size() + indexAddend;
                Double.isNaN(size2);
                desiredTotalTempDeltaForIndex = size2 * tempStep;
        ...
        double d2 = count;
        Double.isNaN(d2);
        int ccwCount = (int) Math.floor((d2 - 1.0d) / 2.0d);
    ...
    private List<Hct> getHctsByTemp() {
        ...
        hcts.add(this.input);
        temperaturesComparator = Comparator.comparing(new Function() { // from class: com.google.android.material.color.utilities.TemperatureCache$$ExternalSyntheticLambda1
            @Override // java.util.function.Function
            public final Object apply(Object obj) {
                return TemperatureCache.this.m106xb7a75f35((Hct) obj);
            }
    ...
    public /* synthetic */ Double m106xb7a75f35(Hct arg) {
