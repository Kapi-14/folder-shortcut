    ...
    static class InnerCornerShadowOperation extends ShadowCompatOperation {
        ...
        public void draw(Matrix transform, ShadowRenderer shadowRenderer, int shadowElevation, Canvas canvas) {
            ...
            if (sweepAngle <= 0.0f) {
                ...
                double d = arcRadius;
                double tan = Math.tan(Math.toRadians((-sweepAngle) / 2.0f));
                Double.isNaN(d);
                ...
                double length12 = arcRadius;
                Double.isNaN(length12);
                matrix.preTranslate((float) ((-retractLength) - length12), (-2.0f) * arcRadius);
                double d2 = arcRadius;
                Double.isNaN(d2);
                double length23 = length2;
                shadowRenderer.drawInnerCornerShadow(canvas, this.renderMatrix, rect, (int) arcRadius, 450.0f, sweepAngle, new float[]{(float) (d2 + retractLength), 2.0f * arcRadius});
